...
export class InMemoryDataService implements InMemoryDbService {
     ...
     createDb(): {} {
        ...
        {question: 'Houve falta de alimentação escolar durante o período letivo? Caso positivo informe o(s) períodos e os produtos?',
         check: false, answer: ''},
        {question: 'A escola conseguiu dubstituir a preparação/produto que faltou por outra,'
        + ' para que não houvesse prejuízo aos alunos? Qual foi a substituição?',
         check: false, answer: ''},
        ...
        {question: 'A quantidade de gêneros entregues nas escolas foi suficiente para ofertar'
        + ' refeições para todos os alunos até que se proceda à próxima distribuição?',
         check: false, answer: ''},
        {question: 'A escola recebe doação de alimentos?', check: false, answer: ''},
    ];

        const formCheckAnswer: Array<FormCheck> = [
